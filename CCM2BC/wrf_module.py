...
def get_met_file_by_time(time):
    return f"met_em.d0{config.domain}."+time+".nc"
...
#   Only if you are going to update wrfbdy
    ...
    if config.do_BC:
        ...
        #xlon=met_files[0].variables['XLONG'][0,:]
        #xlat=met_files[0].variables['XLAT'][0,:]
    ...
    if config.do_IC:
        ...
        wrfinput=Dataset(config.wrf_dir+"/"+config.wrf_input_file,'r')
        for i in range(0,len(wrfinput.variables['Times'][:]),1):
            wrftime = codecs.utf_8_decode(b''.join(wrfinput.variables['Times'][i]))[0]
            wrf_times.update({wrftime:i})
            met_times_files.update({wrftime:met_files[i]})
