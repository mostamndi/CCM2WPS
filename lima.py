...
for itime in range(Ntim):
    ...
    for k, w in enumerate(var_3d):
        ...
        for m, l in enumerate(levels):
            ...
            ccm.data = v3d[m].reshape([Nlat, Nlon])
            ccm.writeFile()
...
"""
T = ttt.reshape([Ntim, NLev, Nlat, Nlon])
for i in range(NLev):
    ccm.setLevel(levels[i]*1000.) # convert levels to standard isobaric levels
    ccm.data = T[0,i]             # Array should be a numpy array
    #ccm.writeFile()
    ccm.write_to_WPS()
    print(i)
ccm.closeFile()                   # Close File
"""